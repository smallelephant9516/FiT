...
import numpy as np
import pandas as pd
from Data_loader import EMData as EMdata
#plotting
...
def main(args):
    with st.sidebar:    # sidebar at the left of the screen
        ...
        dataframe, corpus, helix_name, positive_label, optics= getdata(meta_path)
        ...
        if separate:
            #corpus_ignore=get_corpus(corpus)
            pt(dataframe, corpus, filament_cluster_number, filament_pd['label'], optics, separate, output_path, file_name)
            separate=False
...
def pt(dataframe, corpus, filament_cluster_number,umap_predict, optics, separate, output_path, file_name,datatype=0):
    if separate is True:
...
def getdata(meta_path,datatype=0,N2D=False):
    ...
    return dataframe, corpus, helix_name, positive_label, optics
